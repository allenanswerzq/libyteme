...
struct tensor_view {
 ...
 public:
  ...
  // Get the `idx`th data from first dimension of this tensor.
  template <int D = NDIMS>
  std::enable_if_t<(0 < D), tensor_view<T, NDIMS - 1>> operator[](
      int idx) const {
...
struct tensor {
 ...
 public:
  ...
  template <int D = NDIMS>
  std::enable_if_t<(0 < D), tensor_view<const T, NDIMS - 1>> operator[](
      int idx) const {
    return view()[idx];
  }
  template <int D = NDIMS>
